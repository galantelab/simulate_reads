package App::SimulateReads::IntervalTree;
# ABSTRACT: AVL Interval Binary Tree

use App::SimulateReads::Base 'class';
use App::SimulateReads::IntervalTree::Node;
...
sub insert {
	...
	my $node = App::SimulateReads::IntervalTree::Node->new(
		low  => $low,
		high => $high,
		data => $data
	);
...
sub search {
	...
	$self->_search($self->root, $low, $high, \@nodes);

	return \@nodes;
}

sub _search {
	my ($self, $root, $low, $high, $nodes_a) = @_;
...
sub delete {
	...
	$self->root($self->_delete($self->root, $low, $high, \$node));

	return $node;
}

sub _delete {
	my ($self, $root, $low, $high, $node) = @_;
